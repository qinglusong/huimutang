...
if ($rec == 'backup') {
    ...
    if (trim($sqldump)) {
        $sqldump = "-- 慧目堂设计 v1.x SQL Dump Program\n" . "-- " . ROOT_URL . "\n" . "-- \n" . "-- DATE : " . date('Y-m-d H:i:s') . "\n" . "-- MYSQL SERVER VERSION : " . $dou->version() . "\n" . "-- PHP VERSION : " . PHP_VERSION . "\n" . "-- 慧目堂设计 VERSION : " . $_CFG['慧目堂设计_version'] . "\n\n" . $sqldump;
        ...
        $dou->dou_msg($_LANG['backup_file_success'], 'backup.php?rec=' . $rec . '&vol_size=' . $vol_size . '&totalsize=' . $totalsize . '&filename=' . $filename . '&token=' . $_REQUEST['token'] . '&tableid=' . $tableid . '&fileid=' . $fileid . '&startfrom=' . $startrow, '', 1);
    } else {
...
if ($rec == 'import') {
    ...
    if ($match) {
        ...
        if (file_exists($file_path)) {
            ...
            $fileid++;
            $dou->dou_msg($restore_now, "backup.php?rec=" . $rec . "&sql_filename=" . $sql_filename . "&fileid=" . $fileid);
        } else {
