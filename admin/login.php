...
elseif ($rec == 'login') {
    ...
    if ($_CFG['captcha']) { // 判断是否有开启验证码功能
        $captcha = $check->is_captcha(trim($_POST['captcha'])) ? strtoupper(trim($_POST['captcha'])) : ''; // 判断验证码格式是否正确
        if (md5($captcha . DOU_SHELL) != $_SESSION['captcha']) // 判断验证码是否正确
    ...
    // 验证用户存在和密码
    $user = $dou->fetch_array($dou->select($dou->table('admin'), '*', "user_name = '$user_name'"));
...
elseif ($rec == 'password_reset_post') {
    ...
    if ($action == 'default') { // 密码找回提交操作
        // 根据用户名和邮箱获取对应的用户信息
        $user = $dou->fetch_array($dou->select($dou->table('admin'), '*', "user_name = '$user_name' AND email = '$email'"));
