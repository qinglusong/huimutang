...
class Action extends Manager {
    ...
    function get_sort($module, $field = 'name') {
        ...
        $sort['xiangmuzixun_handle'] = $_SESSION['if_xiangmuzixun_sort'];
     ...
     * +----------------------------------------------------------
     */
    function get_xiangmuzixun_sort($module, $field = 'name') {
        $display = $GLOBALS['_DISPLAY']['home_' . $module];
        ...
        // 项目列表
        $limit = $display ? ' LIMIT ' . $display : '';
        $sql = "SELECT id, " . $field . ", image FROM " . $this->table($module) . " WHERE xiangmuzixun_sort > 0 ORDER BY xiangmuzixun_sort DESC" . $limit;
        $query = $this->query($sql);
        while ($row = $this->fetch_array($query)) {
            $image = ROOT_URL . $row['image'];
            $sort['list'][] = array (
    ...
    function del_file($number) {
        ...
        $this->delete($this->table('file'), "number = '$number'");
    }
    ...
    function dou_api() {
        ...
        $apiget = "ver=$_CFG[慧目堂设计_version]&update=$sys_info[update]&patch=$sys_info[patch]&lang=$_CFG[language]&php_ver=$sys_info[php_ver]&mysql_ver=$sys_info[mysql_ver]&os=$sys_info[os]&web_server=$sys_info[web_server]&charset=$sys_info[charset]&template=$_CFG[site_theme]&url=" . ROOT_URL;
...
/**
     ...
     * +----------------------------------------------------------
     */
function xiangmuzixun_sort_box($module, $act, $id = '') {
        if ($act == 'handle') { // 打开筛选功能
            $_SESSION['if_xiangmuzixun_sort'] = $_SESSION['if_xiangmuzixun_sort'] ? false : true;
        } else {
            // 验证并获取合法的ID
            $id = $GLOBALS['check']->is_number($_REQUEST['id']) ? $_REQUEST['id'] : $this->dou_msg($GLOBALS['_LANG']['illegal'], $module . '.php');
            if ($act == 'set') { // 设为项目咨询显示案例
                $max_sort = $this->get_one("SELECT xiangmuzixun_sort FROM " . $this->table($module) . " ORDER BY xiangmuzixun_sort DESC");
                $new_sort = $max_sort + 1;
                $this->query("UPDATE " . $this->table($module) . " SET xiangmuzixun_sort = '$new_sort' WHERE id = '$id'");
            } elseif ($act == 'cancel') { // 取消首页显示文章
                $this->query("UPDATE " . $this->table($module) . " SET xiangmuzixun_sort = '' WHERE id = '$id'");
            }
        } 
    }
}
