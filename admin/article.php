...
if ($rec == 'default') {
    ...
    $smarty->assign('xiangmuzixun_sort', $dou->get_xiangmuzixun_sort('article', 'title'));
...
elseif ($rec == 'update') {
    ...
    $sql = "UPDATE " . $dou->table('article') . " SET cat_id = '$_POST[cat_id]', title = '$_POST[title]', defined = '$_POST[defined]' ,content = '$_POST[content]'" . $image . ", keywords = '$_POST[keywords]', description = '$_POST[description]' WHERE id = '$_POST[id]'";
    $dou->query($sql);
...
elseif ($rec == 'xiangmuzixun_sort') {
    ...
    $dou->xiangmuzixun_sort_box('article', $act, $id);
    $dou->dou_header($_SERVER['HTTP_REFERER']); // 跳转到上一页面
}
